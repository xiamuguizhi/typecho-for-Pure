...
function themeConfig($form) {
	...
	//favicon图标
	$favicon = new Typecho_Widget_Helper_Form_Element_Text('favicon', NULL, NULL, _t('favicon图标'), _t('输入图片 URL 地址, 显示favicon图标，留空显示默认。'));
	$form->addInput($favicon);

	//首页LOGO
	$logo = new Typecho_Widget_Helper_Form_Element_Text('logo', NULL, NULL, _t('首页LOGO'), _t('输入图片 URL 地址, 在网站首页显示LOGO，留空显示默认。'));
	$form->addInput($logo);
	
	//个人资料图标描述
	$summary = new Typecho_Widget_Helper_Form_Element_Text('summary', NULL, NULL, _t('首页网站描述'), _t('输入一段描述, 位于首页大名称下方，留空默认显示站点描述。'));
    ...
    $top = new Typecho_Widget_Helper_Form_Element_Radio('top',
        ...
        'disable', _t('返回顶部'), _t('默认禁止,样式为拉姆雷姆'));
    $form->addInput($top);
    ...
    $f12 = new Typecho_Widget_Helper_Form_Element_Radio('f12',
        ...
        'disable', _t('禁止F12'), _t('禁止 F12 / 审查开启控制台'));
    $form->addInput($f12);

	//百度自动推送
    $baidukai = new Typecho_Widget_Helper_Form_Element_Radio('baidukai',
        array('able' => _t('启用'),
            'disable' => _t('禁止'),
        ),
        'disable', _t('百度自动推送'), _t('接口调用地址请到百度资源平台申请,然后修改 comments.php 文件 140 行  接口地址在开启 否则 打开页面将 卡死。默认不使用'));
    $form->addInput($baidukai);	

	//手动置顶
	$sticky = new Typecho_Widget_Helper_Form_Element_Textarea('sticky', NULL, NULL, _t('手动置顶文章'), _t('复制首页文章要置顶的代码到这里,就是这么简单<span class="sticky-top-flag gt-bg-accent-color-first">置顶</span>'));
	$form->addInput($sticky);

	//GitHub
	$GitHub = new Typecho_Widget_Helper_Form_Element_Text('GitHub', NULL, NULL, _t('Github'), _t('Github用户地址，留空则无。'));
	$form->addInput($GitHub);

	//weibo微博
	$weibo = new Typecho_Widget_Helper_Form_Element_Text('weibo', NULL, NULL, _t('微博'), _t('微博用户地址，留空则无。'));
	$form->addInput($weibo);

	//ICP备案号
	$ICPbeian = new Typecho_Widget_Helper_Form_Element_Text('ICPbeian', NULL, NULL, _t('ICP备案号'), _t('输入ICP备案号，留空则不显示。'));
	$form->addInput($ICPbeian);
		
	//底部自定义
	$footerContent = new Typecho_Widget_Helper_Form_Element_Textarea('footerContent', NULL, NULL, _t('底部自定义内容'), _t('位于底部，footer之内，适合放置一些JS内容，如网站统计、备案信息代码等。'));
...
//Typecho PHP正则提取img标签src和alt  评论新窗口打开 <a href="$1" data-title="$2" data-lightbox="roadtrip" /><img alt="$2" src="$1"></a>   
function parseContent($obj){
    ...
    $obj->content = preg_replace('/<\s*img[\s\S]+?(?:src=[\'"]([\S\s]*?)[\'"]\s*|alt=[\'"]([\S\s]*?)[\'"]\s*|[a-z]+=[\'"][\S\s]*?[\'"]\s*)+[\s\S]*?>/i','<img src="https://cdn.jsdelivr.net/gh/xiamuguizhi/oss/img/lazyload.jpg" alt="$2" data-src="$1">',$obj->content);
    $obj->content = preg_replace("/<a href=\"([^\"]*)\">/i", "<a href=\"\\1\" target=\"_blank\">", $obj->content);   
	echo trim($obj->content);
}

//判断是否存在图片 在根据结果切换不同的样式
function yotu($thiz){
